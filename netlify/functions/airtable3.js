...
const TOKEN = process.env.AIRTABLE_TOKEN_3;
const BASE_ID = process.env.AIRTABLE_BASE_ID_3;
...
exports.handler = async (event) => {
  ...
  const parts = event.path.replace('/.netlify/functions/airtable3', '').split('/').filter(Boolean);
  const table = parts[0];
